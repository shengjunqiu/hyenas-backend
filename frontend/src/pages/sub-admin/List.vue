...
import {
  createSubAdminApi,
  getSubAdminsApi,
  resetSubAdminPasswordApi,
  updateSubAdminApi,
  updateSubAdminStatusApi,
} from '@/api/sub-admin'
...
const onSubmit = async () => {
  ...
  try {
    if (isEdit.value) {
      await updateSubAdminApi(form.id, {
        name: form.name,
        phone: form.phone || undefined,
      })
      ElMessage.success('子管理员更新成功')
    } else {
...
const onToggle = async (row: Admin, value: boolean) => {
  ...
  try {
    await updateSubAdminStatusApi(row.id, targetStatus)
...
const onConfirmResetPassword = async () => {
  ...
  try {
    await resetSubAdminPasswordApi(passwordForm.id, passwordForm.newPassword)
...
<template>
  <div class="page-block">
    ...
    <div style="margin-bottom: 12px">
      <el-button type="primary" @click="onCreate">新增子管理员</el-button>
    </div>
